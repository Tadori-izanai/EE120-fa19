\vspace{.5em}
\begin{tikzpicture}[scale = .45]
    \small
    %% axis
    \draw[-latex', thick] (0, -2.5) -- (0, 2.5) node[right = 0] {$20\lg |H(j\omega)|$ / dB};
    \draw[-latex', thick] (0, 0) node[left = 0] {$O$} 
        -- (10, 0) node[right = 0] {$\omega$};

    %% scale
    \draw (2, 0) node[below = 0] {\tiny $\frac{1}{10}$} -- +(0, 0.3);
    \draw (4, 0) node[below = 0] {\tiny $1$} -- +(0, 0.3);
    \draw (6, 0) node[below = 0] {\tiny $10$} -- +(0, 0.3);
    \draw (8, 0) node[below = 0] {\tiny $100$} -- +(0, 0.3);
    \draw (0, 1) node[left = 0] {\tiny $\phantom{-0^\circ}20$} -- +(0.3, 0);
    \draw (0, -1) node[left = 0] {\tiny $-20$} -- +(0.3, 0);

    %% curve
    \draw[blue, thick] (0, 0) -- (9, 0);
\end{tikzpicture} 

\begin{tikzpicture}[scale = .45]
    \small
    %% axis
    \draw[-latex', thick] (0, -4) -- (0, 1) node[right = 0] {$\arg H(j\omega)$};
    \draw[-latex', thick] (0, 0) node[left = 0] {$O$} 
        -- (10, 0) node[right = 0] {$\omega$};

    %% scale
    \draw (2, 0) node[below = 0] {\tiny $\frac{1}{10}$} -- +(0, 0.3);
    \draw (4, 0) node[below = 0] {\tiny $1$} -- +(0, 0.3);
    \draw (6, 0) node[below = 0] {\tiny $10$} -- +(0, 0.3);
    \draw (8, 0) node[below = 0] {\tiny $100$} -- +(0, 0.3);
    \draw (0, -1.5) node[left = 0] {\tiny $-90^\circ$} -- +(0.3, 0);
    \draw (0, -3.0) node[left = 0] {\tiny $-180^\circ$} -- +(0.3, 0);

    %% curve
    \draw[blue, thick] (0, 0) -- (2, 0) (2, 0) -- (6, -3) (6, -3) -- +(3, 0);
    
\end{tikzpicture}
